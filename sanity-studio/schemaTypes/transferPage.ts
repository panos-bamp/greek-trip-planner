...
export default defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: 'number',
      group: 'content',
      description: 'Controls row order on /transfers (lower = higher up). Hubs only — use your Tier 1–4 priority (e.g. Athens: 1, Santorini: 2, Mykonos: 3...). Spokes ignore this.',
      hidden: ({parent}) => parent?.pageType !== 'hub',
    }),
    defineField({
      name: 'primaryRouteLabel',
      title: 'Primary Route Label',
      type: 'string',
      group: 'content',
      description: 'Hubs only — the single representative route shown in the /transfers arrivals board, e.g. "Airport → City Centre". This is deliberately not every route the hub covers, just the board teaser.',
      hidden: ({parent}) => parent?.pageType !== 'hub',
    }),
    defineField({
      name: 'parentHub',
    ...
    defineField({
      name: 'bookingUrl',
      title: 'Welcome Pickups Booking URL',
      type: 'url',
      group: 'content',
      description: 'Route-specific Travelpayouts deep link for THIS route — not the generic Welcome Pickups homepage link. This is what actually renders in the booking card.',
    }),
    defineField({
      name: 'localOperators',
      ...
      type: 'array',
      group: 'content',
      description: 'Empty until local operator partnerships go live — the booking card shows a placeholder slot until at least one entry exists here. Cap: 2 on hubs, 1 on spokes (enforced in the component, not here).',
      of: [
        {
          type: 'object',
          fields: [
            {name: 'name', type: 'string', title: 'Operator Name'},
            {name: 'url', type: 'url', title: 'Booking URL (route-specific if possible)'},
            {name: 'price', type: 'string', title: 'Price (optional)'},
            {name: 'ratingLabel', type: 'string', title: 'Rating label (optional)', description: 'e.g. "4.6/5 · 1,200 reviews" — only use real, sourced numbers.'},
          ],
    ...
    defineField({
      ...
      fields: [
        ...
        {name: 'authorName', title: 'Author Name', type: 'string', initialValue: 'Panagiotis'},
        {name: 'authorUrl', title: 'Author URL', type: 'url', initialValue: 'https://greektriplanner.me/about'},
